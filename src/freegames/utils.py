""" 实用工具集
    通常用于存放一些通用的、可复用的工具函数和类，便于维护和管理
    在调用时要使用相对导入  from .utils import floor
    对于该项目中，在 __init__.py 文件中已经全部导入了 utils.py 中的所有函数
        所以在编写游戏时直接使用命令 from freegames import vector
"""
